...
<script>
    ...
    if response:ok() then
        ...
        local noteToPost = "patroclus sucks"
        local time = Time.now()
        ...
        trace.log(JSON.stringify(eventArray))

        for i, relay in pairs(relays) do
            local ws = WebSocket.new(relay)
            ws:on('open', function()
                trace.log('Connected to relay: ' .. relay)
                trace.log('gorl: ' .. string.gsub(JSON.stringify({"EVENT", eventObject}), '(%d+)%.0([^%d])', '%1%2'))
                ws:send(string.gsub(JSON.stringify({"EVENT", eventObject}), '(%d+)%.0([^%d])', '%1%2'))
                trace.log('sent i think')
            end)
